...
- **Extended to K the same day K/DST support and this real-data work were
  merged together** — K's `AVG_GAMES_PLAYED` entry had been a flat guessed
  `16` (see the K/DST section below). Rather than let a real-data pipeline
  and a guessed constant coexist for two different BEER positions,
  `build-games-played-data.js` now fetches K alongside QB/RB/WR/TE —
  confirmed live that Sleeper's stats endpoint carries the same
  `gp`/`pos_rank_ppr` fields for kickers — producing a real
  `GAMES_PLAYED_CURVE.K`. Re-running the script reproduced QB/RB/WR/TE's
  existing curves byte-identical (confirming the historical data is stable
  and deterministic) while adding K's for the first time. **Real finding for
  K**: kickers play almost every game through roughly the top 30 (streaming/
  committee kickers are rare at this position, unlike RB/WR), then fall off
  a cliff sharper than any other position's — resulting `REPLACEMENT_RANK.K`
  for this league's shape is **11**. The flat `AVG_GAMES_PLAYED` constant is
  gone entirely now, not just for K — every BEER position (K included) goes
  through `gamesPlayedAt()`/`GAMES_PLAYED_CURVE`.
...
- **K's games-played comes from the same real-data pipeline as QB/RB/WR/TE
  (see "Real games-played data" below), not a guess** — this was originally
  built as a flat `AVG_GAMES_PLAYED.K = 16` judgment call, then upgraded
  the same day the two features were merged together: confirmed live that
  Sleeper's stats endpoint carries the same `gp`/`pos_rank_ppr` fields for
  kickers, so `build-games-played-data.js` now fetches K alongside
  QB/RB/WR/TE and produces a real `GAMES_PLAYED_CURVE.K`. The flat
  `AVG_GAMES_PLAYED` constant this used to reference is gone entirely
  (superseded for every position, not just K) — see "Real games-played
  data" below for the full writeup, including the real finding that emerged
  for K specifically (a much sharper games-played cliff than any other
  position).
- **`BEER_POSITIONS`** (`[...CORE_POSITIONS, "K"]`) is the actual list
  ...
  curve/starters entry to solve against).
